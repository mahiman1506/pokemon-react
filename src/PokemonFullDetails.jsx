...
const PokemonFullDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchSimilarTypePokemon = async () => {
      ...
      try {
        ...
        const allPokemon = results.flatMap((result) =>
          result.pokemon.map((p) => p.pokemon),
        );
        const uniquePokemon = Array.from(
          new Map(allPokemon.map((p) => [p.name, p])).values(),
        );

        // Remove the current Pokémon from suggestions
        const filtered = uniquePokemon
  ...
  const capitalizedName = pokemonData?.name
  ...
  return (
    <>
      <Helmet>
        <title>
          {capitalizedName
            ? `${capitalizedName} Stats | PokSphere`
            : "Pokemon | PokSphere"}
        </title>
        ...
        <meta
          ...
          content={
            capitalizedName
              ? `Explore ${capitalizedName} stats, abilities and evolution in PokSphere.`
              : "Explore Pokemon stats, abilities and evolutions."
          }
        />

        <meta property="og:title" content={`${capitalizedName} | PokSphere`} />
        ...
        <div className="image-gallery">
          <div className="thumbnails">
            ...
            <img
              ...
              alt={pokemonData.name}
              onClick={() =>
                setMainImage(pokemonData.sprites.other.home.front_default)
              }
            />
            <img
              ...
              alt={pokemonData.name}
              onClick={() =>
                setMainImage(pokemonData.sprites.other.home.front_shiny)
              }
            />
            <img
      ...
      <div className="suggestion">
        <h1 className="suggestion-title">Suggested Pokémon of the Same Type</h1>
        <ul className="cards">
          {" "}
          {relatedPokemons.map((curPokemon) => (
            <PokemonCard key={curPokemon.id} pokemonData={curPokemon} />
          ))}
        </ul>
      </div>
      <div className="homeButton-continer">
        <button
